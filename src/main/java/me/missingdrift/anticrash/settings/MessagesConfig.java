...
public class MessagesConfig {
    ...
    public MessagesConfig(FileConfiguration configuration) {
        configuration.addDefaults(ImmutableMap.<String, Object>builder()
                .put("messages.staff-notification", "&7(&eAnticrash&7) &8» &f%player% &7disconnected for flagging &f%checkname%")
                .put("messages.staff-notification-info", "&7(&e%info%&7)")
                .put("messages.disconnect-message", "&7(&eAnticrash&7) &8» &fYou have been disconnected due \n&fto sending harmful packets.")
                .build());

        this.staffNotification = configuration.getString("messages.staff-notification", "&7(&eAnticrash&7) &8» &f%player% &7disconnected for flagging &f%checkname%");
        this.staffNotificationInfo = configuration.getString("messages.staff-notication-info", "&7(&e%info%&7)");
        this.disconnectMessage = configuration.getString("messages.disconnect-message", "&7(&eAnticrash&7) &8» &fYou have been disconnected due \n&fto sending harmful packets.");

        this.componentSerializer = Anticrash.getPlugin().getComponentSerializer();
    }
